...
async function login(){
  ...
  try{
    ...
    if(error)throw error;if(!data?.session)throw new Error('Geen geldige sessie ontvangen.');
    localStorage.setItem(EMAIL_KEY,email);el('password').value='';await loadCloud();
  }catch(e){
    console.error(e);const msg=String(e?.message||e||'Inloggen mislukt');
...
async function resume(){
  ...
  try{const {data,error}=await supabase.auth.getSession();if(error)throw error;if(data?.session){await loadCloud();return}}catch(e){console.error(e)}
  showLogin();
}
...
async function loadCloud(){
  cloud('Cloud: laden…');
  try{
    const {data:{user},error:userError}=await supabase.auth.getUser();if(userError)throw userError;if(!user)throw new Error('Geen ingelogde gebruiker gevonden.');
    const {data:rows,error}=await supabase.from('hdm_app_state').select('data,updated_at').eq('user_id',user.id).limit(1);if(error)throw error;
    db=rows?.[0]?.data||{};for(const k of ['customers','invoices','receipts','payments','orders'])if(!Array.isArray(db[k]))db[k]=[];
...
function renderDashboard(){
  ...
  const revenue=inv.reduce((a,i)=>a+num(i.total),0);
  const open=inv.reduce((a,i)=>a+Math.max(0,num(i.total)-invoicePaid(i)),0);
  const costs=rec.reduce((a,r)=>a+num(r.total),0);
  const vatReceipts=rec.filter(q3).reduce((a,r)=>a+verifiedReceiptVat(r),0);
  const vatInvoices=inv.filter(q3).reduce((a,i)=>a+(num(i.vatAmount)||vatIncl(i.total)),0);
...
function renderBook(){
  const inv=(db.invoices||[]).filter(q3),rec=(db.receipts||[]).filter(q3);
  const sales=inv.reduce((a,i)=>a+num(i.total),0);
  const vatOut=inv.reduce((a,i)=>a+(num(i.vatAmount)||vatIncl(i.total)),0);
  const vatIn=rec.reduce((a,r)=>a+verifiedReceiptVat(r),0);
  const review=reviewReceipts().length;
  el('bookCards').innerHTML=[['Omzet Q3',euro(sales)],['BTW klantfacturen Q3',euro(vatOut)],['BTW bonnetjes Q3 · gecontroleerd',euro(vatIn)],['BTW-bonnen te controleren',review+' bonnen']].map(c=>'<div class="card metric"><small>'+c[0]+'</small><strong>'+c[1]+'</strong></div>').join('');
  el('bookText').innerHTML='BTW klantfacturen Q3: <strong>'+euro(vatOut)+'</strong><br><br>Gecontroleerde BTW bonnetjes Q3: <strong>'+euro(vatIn)+'</strong><br><br>Voorlopig verschil: <strong>'+euro(vatOut-vatIn)+'</strong><br><br><span class="muted">'+review+' bon(nen) tellen nog niet mee omdat het btw-bedrag niet betrouwbaar uit het document kon worden vastgesteld.</span>'
}
